...
CREATE PROC sp_ViewReservedEquipment
(
	@TransactionID int
)
as
SELECT TBLEQUIPMENTRESERVATION.TransactionID, TBLEQUIPMENTRESERVATION.EQUIPMENT_ID, TBLEQUIPMENTDETAILS.EQUIPMENT_NAME, TBLEQUIPMENTRESERVATION.Quantity FROM TBLEQUIPMENTRESERVATION
INNER JOIN TBLEQUIPMENTDETAILS ON TBLEQUIPMENTRESERVATION.EQUIPMENT_ID=TBLEQUIPMENTDETAILS.EQUIPMENT_ID
WHERE TBLEQUIPMENTRESERVATION.TransactionID=@TransactionID
...
CREATE TABLE TBLPULLEDOUTEQUIPMENT
(
	PullOutID INT UNIQUE IDENTITY(0,1),
	EQUIPMENT_ID INT FOREIGN KEY REFERENCES TBLEQUIPMENTDETAILS(EQUIPMENT_ID),
	QUANTITY INT
)

CREATE PROC sp_PullOutNewEquipment
(
	@EQUIPMENT_ID INT,
	@QUANTITY INT
)
AS
INSERT INTO TBLPULLEDOUTEQUIPMENT
VALUES (@EQUIPMENT_ID, @QUANTITY)

CREATE PROC sp_PullOutEquipment
(
	@EQUIPMENT_ID INT,
	@QUANTITY INT
)
AS
UPDATE TBLPULLEDOUTEQUIPMENT
SET TBLPULLEDOUTEQUIPMENT.QUANTITY=@QUANTITY
WHERE TBLPULLEDOUTEQUIPMENT.EQUIPMENT_ID=@EQUIPMENT_ID
...
VALUES	('Computer Laboratory 1', 1000002, 'Computer Laboratory', '1CL1'),
		...
		('Computer Laboratory 4', 1000001, 'Computer Laboratory', '1CL4')
...
CREATE PROC sp_ViewDeployedEquipmentsTransactionID
(
	@TransactionID int
)
as
SELECT TBLEQUIPMENTDETAILS.EQUIPMENT_ID as 'ID', TBLEQUIPMENTDETAILS.EQUIPMENT_NAME as 'Name', TBLBORROWED.Quantity, TBLBORROWED.IsGoodCondition as 'Returned?'FROM TBLBORROWED
INNER JOIN TBLEQUIPMENTDETAILS ON TBLBORROWED.EQUIPMENT_ID=TBLEQUIPMENTDETAILS.EQUIPMENT_ID
INNER JOIN TBLTRANSACTION ON TBLBORROWED.TransactionID=TBLTRANSACTION.TransactionID
WHERE TBLTRANSACTION.TransactionID=@TransactionID

CREATE PROC sp_ViewDeployedEquipmentsFacilityID
(
	@FacilityID int
)
as
SELECT DISTINCT TBLEQUIPMENTDETAILS.EQUIPMENT_ID as 'ID', TBLEQUIPMENTDETAILS.EQUIPMENT_NAME as 'Name', TBLBORROWED.Quantity, TBLBORROWED.IsGoodCondition as 'Returned?'FROM TBLBORROWED
INNER JOIN TBLEQUIPMENTDETAILS ON TBLBORROWED.EQUIPMENT_ID=TBLEQUIPMENTDETAILS.EQUIPMENT_ID
INNER JOIN TBLTRANSACTION ON TBLBORROWED.TransactionID=TBLTRANSACTION.TransactionID
INNER JOIN TBLDEPLOYMENT ON TBLBORROWED.TransactionID=TBLDEPLOYMENT.TransactionID
INNER JOIN TBLFACILITIES ON TBLDEPLOYMENT.FacilitiyID=TBLFACILITIES.FacilityID
WHERE TBLFACILITIES.FacilityID=@FacilityID
